...
// Lấy thông tin bài test, bao gồm cả Pass và số câu hiển thị
$stmt = $conn->prepare("SELECT id_test, ten_test, lan_thu, Pass, so_cau_hien_thi FROM test WHERE id_test = ?");
$stmt->bind_param("i", $id_test);
$stmt->execute();
$result = $stmt->get_result();
...
$required_pass_percent = is_numeric($test_info['Pass']) ? intval($test_info['Pass']) : 80; // Lấy từ DB, fallback 80 nếu lỗi
$so_cau_hien_thi = $test_info['so_cau_hien_thi'] ?? 0;
$stmt->close();
    ...
    <div class="container">
        ...
        <?php if ($recent_result): ?>
            ...
            <table class="result-table">
                ...
                <tr><td>Điểm cao nhất:</td><td><?php echo $recent_result['kq_cao_nhat']; ?> / <?php echo $so_cau_hien_thi; ?></td></tr>
        ...
        <?php else: ?>
            ...
            <table class="test-info-table">
                <tr><td>Khóa học:</td><td><?php echo htmlspecialchars($id_khoa); ?></td></tr>
                <tr><td>Bài test: </td><td> <?php echo htmlspecialchars($ten_test); ?></td></tr>
                <tr><td>Số câu hỏi:</td><td><?php echo $so_cau_hien_thi; ?></td></tr>
