...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["save_question"])) {
    $id_khoa = (int)trim($_POST["id_khoa"]);
    $id_baitest = (int)trim($_POST["id_baitest"]);
    ...
    if (empty($id_khoa) || empty($id_baitest) || empty($question_text) ||
        ...
        if ($question_id) {
            // UPDATE query
            $sql = "UPDATE quiz SET id_baitest=?, id_khoa=?, cauhoi=?, hinhanh=?, 
                    ...
                    WHERE Id_cauhoi=?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("iissssssssssssssis", 
                $id_baitest, $id_khoa, $question_text, $image,
        ...
        if ($stmt->execute()) {
            $stmt->close();
            $conn->close();
            header("Location: ".$_SERVER['PHP_SELF']."index.php?add_question&success=1");
            exit;
        } else {
                      ...
                      <option value="">-- Chọn khóa học --</option>
                        <?php foreach($khoa_hoc_list as $khoa): ?>
                            <option value="<?= $khoa['id'] ?>" 
                                <?= ($question_id && $question_data['id_khoa'] == $khoa['id']) ? 'selected' : '' ?>>
