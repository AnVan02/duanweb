...
if (isset($_GET['delete_id']) && is_numeric($_GET['delete_id']) && $id_test > 0) {
    ...
    if ($result->num_rows > 0) {
        ...
        $id_khoa = $row['id_khoa']; // Lấy id_khoa
        
        // Kiểm tra và xóa hình ảnh nếu có
        $sql = "SELECT hinhanh, hinhanh_a, hinhanh_b, hinhanh_c, hinhanh_d 
                FROM quiz 
                WHERE Id_cauhoi = ? AND id_baitest = ? AND id_khoa = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iii", $delete_id, $id_test, $id_khoa); // Sử dụng id_khoa
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        // Xóa câu hỏi
        $sql = "DELETE FROM quiz WHERE Id_cauhoi = ? AND id_baitest = ? AND id_khoa = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iii", $delete_id, $id_test, $id_khoa); // Sử dụng id_khoa
        if ($stmt->execute()) {
...
if ($id_test > 0 && $test_info && $khoa_hoc) {
    $conn = dbconnect();
    $sql = "SELECT q.Id_cauhoi, q.id_baitest, q.id_khoa, q.cauhoi, q.hinhanh, 
