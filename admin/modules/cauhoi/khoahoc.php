...
// Sử dụng kết nối từ admin config
global $mysqli;
$conn = $mysqli;
...
// Xử lý xóa nhiều bài kiểm tra
if (isset($_POST['delete_selected']) && $id_khoa > 0 && isset($_POST['selected_tests'])) {
    ...
    if ($deleted_count > 0) {
        $success_message = "<p>Đã xóa thành công $deleted_count bài kiểm tra!</p>";
    } else {
        $error_message = "<p>Không có bài kiểm tra nào được xóa!</p>";
    }
}

// Xử lý xóa bài kiểm tra đơn lẻ
if (isset($_GET['delete_test']) && $id_khoa > 0) {
    $test_id = (int)$_GET['delete_test'];
    $stmt = $conn->prepare("DELETE FROM test WHERE id_test = ? AND id_khoa = ?");
    if ($stmt) {
        // Sửa biến truyền vào bind_param
        $stmt->bind_param("ii", $test_id, $id_khoa);
        if ($stmt->execute()) {
            echo "<script>window.location.href = '" . $_SERVER['PHP_SELF'] . "?action=khoahoc&id_khoa=" . $id_khoa . "';</script>";
            exit();
        } else {
...
if (isset($_GET['edit_test']) && $id_khoa > 0) {
    ...
    if ($id_test > 0) {
        ...
        $error_message = "<p>ID bài kiểm tra không hợp lệ.</p>";
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_test']) && $id_khoa > 0) {
    ...
    // echo "Debug - ID Test: $id_test, ID Khoa: $id_khoa<br>";
    
    // Validate dữ liệu
    if ($id_test <= 0) {
        $error_message = "<p>Lỗi: ID bài kiểm tra không hợp lệ (ID = $id_test)!</p>";
    } elseif (empty($ten_test)) {
        ...
        $sql_count = "SELECT COUNT(q.Id_cauhoi) as so_cau FROM quiz q INNER JOIN khoa_hoc k ON LOWER(TRIM(q.id_khoa)) = LOWER(TRIM(k.khoa_hoc)) WHERE k.id = ? AND q.id_baitest = ?";
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    if ($stmt->affected_rows > 0) {
                        ...
                        // Redirect sau 2 giây để người dùng thấy thông báo
                        echo "<script>
                            setTimeout(function() {
                                window.location.href = '" . $_SERVER['PHP_SELF'] . "?action=khoahoc&id_khoa=" . $id_khoa . "';
                            }, 2000);
                        </script>";
                    } else {
                        $error_message = "<p>Không có dữ liệu nào được cập nhật. Có thể bài test không tồn tại hoặc không có thay đổi.</p>";
                    }
                } else {
                ...
                $error_message = "<p>Lỗi chuẩn bị câu lệnh SQL: " . $conn->error . "</p>";
            }
        }
    }
}
...
if ($id_khoa > 0 && $khoa_hoc) {
    $sql = "SELECT t.id_test, t.id_khoa, t.ten_test, t.lan_thu, t.pass, t.so_cau_hien_thi, k.khoa_hoc FROM test t LEFT JOIN khoa_hoc k ON t.id_khoa = k.id WHERE t.id_khoa = ? ORDER BY t.id_test";
...
<body>
    ...
    <a href="<?php echo htmlspecialchars($_SERVER['PHP_SELF'] . '?action=add_khoahoc'); ?>" class="back-button">Quay lại danh sách khóa học</a>
    
    <h2>Danh sách bài test</h2>
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?action=khoahoc&id_khoa=" . $id_khoa); ?>">
            ...
            <?php if ($editing): ?>
                ...
                <a href="<?php echo htmlspecialchars($_SERVER['PHP_SELF'] . '?action=khoahoc&id_khoa=' . $id_khoa); ?>" class="cancel-button">Hủy</a>
            <?php else: ?>
    ...
    <?php if ($khoa_hoc && $id_khoa > 0): ?>
        <!-- Form xóa nhiều -->
        <form id="delete-multi-form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?action=khoahoc&id_khoa=" . $id_khoa); ?>">
                ...
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="<?php echo htmlspecialchars($_SERVER['PHP_SELF'] . '?action=khoahoc&id_khoa=' . $id_khoa . '&edit_test=' . $row['id_test']); ?>" class="edit-button">Sửa</a>
                                <a href="<?php echo htmlspecialchars($_SERVER['PHP_SELF'] . '?action=khoahoc&id_khoa=' . $id_khoa . '&delete_test=' . $row['id_test']); ?>" class="delete-button" onclick="return confirm('Bạn có chắc chắn muốn xóa bài kiểm tra này?')">Xóa</a>
                                <a href="<?php echo htmlspecialchars($_SERVER['PHP_SELF'] . '?action=question&id_test=' . $row['id_test']); ?>" class="action-button">Xem câu hỏi</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
