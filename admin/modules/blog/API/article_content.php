...
if (empty($article_link) || empty($article_content)) {
    die(json_encode([
        "success" => false,
...
$sql = "UPDATE article SET article_content=? WHERE article_link=?";
$stmt = $mysqli->prepare($sql);
$stmt->bind_param("ss", $article_content, $article_link);

if ($stmt->execute()) {
    echo json_encode(["success"=>true,"message"=>"Cập nhật nội dung thành công!"],JSON_UNESCAPED_UNICODE);
} else {
