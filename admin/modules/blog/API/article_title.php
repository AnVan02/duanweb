...
if (empty($article_link) || empty($article_title)) {
    die(json_encode([
        "success" => false,
...
$sql = "UPDATE article SET article_title=? WHERE article_link=?";
$stmt = $mysqli->prepare($sql);
$stmt->bind_param("ss", $article_title, $article_link);

if ($stmt->execute()) {
    echo json_encode(["success"=>true,"message"=>"Cập nhật tiêu đề thành công!"],JSON_UNESCAPED_UNICODE);
} else {
