...
if (empty($article_link) || empty($article_tag)) {
    die(json_encode([
        "success" => false,
...
$sql = "UPDATE article SET article_tag=? WHERE article_link=?";
$stmt = $mysqli->prepare($sql);
$stmt->bind_param("ss", $article_tag, $article_link);

if ($stmt->execute()) {
    echo json_encode(["success"=>true,"message"=>"Cập nhật tag thành công!"],JSON_UNESCAPED_UNICODE);
} else {
