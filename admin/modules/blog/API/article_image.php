...
if (empty($article_link) || empty($article_image)) {
    die(json_encode([
        "success" => false,
...
if (!filter_var($article_image, FILTER_VALIDATE_URL)) {
    die(json_encode(["success"=>false, "message"=>"Link ảnh không hợp lệ!"]));
}

$sql = "UPDATE article SET article_image=? WHERE article_link=?";
$stmt = $mysqli->prepare($sql);
$stmt->bind_param("ss", $article_image, $article_link);

if ($stmt->execute()) {
    echo json_encode(["success"=>true,"message"=>"Cập nhật ảnh thành công!"],JSON_UNESCAPED_UNICODE);
} else {
