...
if (empty($article_link) || empty($article_date)) {
    die(json_encode([
        "success" => false,
...
$d = DateTime::createFromFormat('Y-m-d', $article_date);
if (!$d || $d->format('Y-m-d') !== $article_date) {
    die(json_encode(["success"=>false, "message"=>"Ngày không hợp lệ (YYYY-MM-DD)!"]));
}

$sql = "UPDATE article SET article_date=? WHERE article_link=?";
$stmt = $mysqli->prepare($sql);
$stmt->bind_param("ss", $article_date, $article_link);

if ($stmt->execute()) {
    echo json_encode(["success"=>true,"message"=>"Cập nhật ngày thành công!"],JSON_UNESCAPED_UNICODE);
} else {
    echo json_encode(["success"=>false,"message"=>"Lỗi: ".$stmt->error],JSON_UNESCAPED_UNICODE);
}
?>
