...
if (empty($article_link) || empty($article_summary)) {
    die(json_encode([
        "success" => false,
...
$sql = "UPDATE article SET article_summary=? WHERE article_link=?";
$stmt = $mysqli->prepare($sql);
$stmt->bind_param("ss", $article_summary, $article_link);

if ($stmt->execute()) {
    echo json_encode(["success"=>true,"message"=>"Cập nhật tóm tắt thành công!"],JSON_UNESCAPED_UNICODE);
} else {
