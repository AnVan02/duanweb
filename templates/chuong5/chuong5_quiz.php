...
$id_test = isset($_GET['id_test']) ? $_GET['id_test'] : '9'; // Lấy id_test từ URL
$ma_khoa = '19';
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && (isset($_POST['next']) || isset($_POST['submit']) || isset($_POST['previous']))) {
    ...
    if (isset($_POST['next']) && $current_index < count($_SESSION['questions_' . $id_test]) - 1) {
        ...
        $conn->close();
        header("Location: chuong5_result.php?id_test=$id_test");
        exit();
    }
    header("Location: chuong5_quiz.php?id_test=$id_test");
    exit();
}
...
<body>
    <div class="container">
        ...
        <?php else: ?>
            ...
            <script>window.location.href = 'chuong5_result.php?id_test=<?php echo htmlspecialchars($id_test); ?>';</script>
        <?php endif; ?>
    </div>
</body>
</html>
