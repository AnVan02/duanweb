...
if (isset($_GET['start']) && $_GET['start'] == 1) {
    ...
    header("Location: chuong3_quiz.php?id_test=$id_test");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && (isset($_POST['next']) || isset($_POST['submit']) || isset($_POST['previous']))) {
    ...
    if (isset($_POST['next']) && $current_index < count($_SESSION['questions_' . $id_test]) - 1) {
        ...
        $conn->close();
        header("Location: chuong3_result.php?id_test=$id_test");
        exit();
    }
    header("Location: chuong3_quiz.php?id_test=$id_test");
    exit();
...
<body>
    <div class="container">
        ...
        <?php if ($attempts >= $max_attempts): ?>
            <p class="no-answers">Bạn đã sử dụng hết số lần làm bài! <a class="nav-link" href="chuong3_result.php?id_test=<?php echo htmlspecialchars($id_test); ?>">Xem kết quả</a></p>
        ...
        <?php else: ?>
            ...
            <script>window.location.href = 'chuong3_result.php?id_test=<?php echo htmlspecialchars($id_test); ?>';</script>
        <?php endif; ?>
    </div>
</body>
</html>
