...
foreach ($course_ids as $khoa_id) {
    ...
    $hoan_thanh = ($total_test > 0 && $da_lam >= $total_test);
...
<!-- dạt ohan trăn nhưng co 1 bai chưa dat thi khoá hoan tất -->
 <?php
session_start();
...
foreach ($course_ids as $khoa_id) {
    ...
    while ($test_row = $test_results->fetch_assoc()) {
        $kq_cao_nhat = $test_row['kq_cao_nhat'];
        $so_cau_hien_thi = $test_row['so_cau_hien_thi'];
        $pass_percent = floatval($test_row['pass']) ?? 50; // Nếu không có pass thì mặc định 50%
